...
- Damage the Toxic Sheep and confirm the configured `on_damage` actions run without console errors.
...
- Dedicated in-game checks for `on_tame`, `on_breed`, and `on_interact` examples once sample variants are added for tameable/breedable/interactable mobs.
